...
=== Preventing a record from being deserialized twice
...
or:
    ...
    acts_as_seedable :prevent_duplicate_records => true
...
1. The same record can not be present in the output more than once, or
   it won't be able to be imported.  (Duplicate key constraint
   violation.)  I'm hoping that a open pull request on rails master will
   help solve this issue.
2. Importing objects with associated objects by primary key will only
